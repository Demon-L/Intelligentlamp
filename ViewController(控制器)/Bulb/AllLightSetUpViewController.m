...
- (void)viewDidLoad{
    ...
    [self setTitleViewWithStr:NSLocalizedString(@"Share", nil)];
...
- (void)layoutUI{
    ...
    UILabel *l1 = [[UILabel alloc]initWithFrame:CGRectMake(30 , 154 +(LampCirWidth - 30)/2, LampCirWidth, 30)];
    l1.text =NSLocalizedString(@"Power", nil);
    l1.textAlignment = NSTextAlignmentCenter;
    l1.textColor = [UIColor whiteColor];
    l1.font = [UIFont systemFontOfSize:12];
    [self.scrollView addSubview:l1];
    
    UILabel *l = [[UILabel alloc]initWithFrame:CGRectMake(60 +  LampCirWidth , 154 +(LampCirWidth - 30)/2 , LampCirWidth, 30)];
    l.text =NSLocalizedString(@"Cri", nil);
    l.textAlignment = NSTextAlignmentCenter;
    l.textColor = [UIColor whiteColor];
    l.font = [UIFont systemFontOfSize:12];
    [self.scrollView addSubview:l];
    
    UILabel *l3 = [[UILabel alloc]initWithFrame:CGRectMake(30 , 154 +60 + LampCirWidth+ (LampCirWidth- 30)/2, LampCirWidth, 30)];
    l3.text =NSLocalizedString(@"Tonal", nil);
    l3.textAlignment = NSTextAlignmentCenter;
    l3.textColor = [UIColor whiteColor];
    l3.font = [UIFont systemFontOfSize:12];
    [self.scrollView addSubview:l3];
    
    UILabel *l2 = [[UILabel alloc]initWithFrame:CGRectMake(60 +  LampCirWidth , 154 +60 + LampCirWidth+ (LampCirWidth- 30)/2, LampCirWidth, 30)];
    l2.text =NSLocalizedString(@"Saturation", nil);
...
- (LampCircularSlider *)powerCir{
    if (!_powerCir) {
        ...
        _powerCir.unselectImage = [UIImage imageNamed:@"power"];
...
- (LampCircularSlider *)showCir{
    if (!_showCir) {
        ...
        _showCir.unselectImage = [UIImage imageNamed:@"cri"];
...
- (LampCircularSlider *)tonalCir{
    if (!_tonalCir) {
        ...
        _tonalCir.unselectImage = [UIImage imageNamed:@"tone"];
...
- (LampCircularSlider *)psaturationCir{
    if (!_psaturationCir) {
        ...
        _psaturationCir.unselectImage = [UIImage imageNamed:@"saturation"];
